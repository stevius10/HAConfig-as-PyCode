...
def ha_logs_truncate(trigger_type=None, logpath=HA_PATH_LOG):
  log = ""
  ...
  async with aopen(logpath, 'r') as log_file:
  ...
  if len(log) > (1.5 * log_size):
    ...
    async with aopen(f"{logpath}.archive", 'a') as archive_file:
      await archive_file.writelines(log_to_archive)
    async with aopen(f"{logpath}.archive", 'r') as read_archive_file:
      archive = read_archive_file.readlines()
  
    log_trunc = log[-log_size:]
    archive_trunc = archive[-archive_size:]
  
    async with aopen(logpath, 'w') as log_file:
      await log_file.writelines(log_trunc)
  
    async with aopen(f"{logpath}.archive", 'w') as log_file:
