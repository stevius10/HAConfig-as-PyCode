...
@state_trigger(expr([state.get(f"{entity["fan"]}.percentage") for entity in entities.values()], SCRIPT_AIR_CLEANER_SLEEP_MODE_PERCENTAGE, comparator='>'), state_hold=SCRIPT_AIR_CLEANER_TIMEOUT_CLEAN, watch=[f"{entity["fan"]}.percentage" for entity in entities.values()])
@task_unique("-".join([entity["fan"] for entity in entities.values()]), kill_me=False)
@logged
@service
def script_air_cleaner_sleep(entity=[entity["fan"] for entity in entities.values()], var_name=None, value=None):
  if var_name or not isinstance(entity, list): 
    supported_features = str(state.get("{}.supported_features".format(var_name or entity)))
    if supported_features == "9": # int without str(state.get())
      fan.set_preset_mode(entity_id=(var_name or entity), preset_mode=SCRIPT_AIR_CLEANER_PRESET_MODE_SLEEP)
    elif supported_features == "1":
      fan.turn_on(entity_id=(var_name or entity))
      fan.set_percentage(entity_id=(var_name or entity), percentage=SCRIPT_AIR_CLEANER_SLEEP_MODE_PERCENTAGE)
