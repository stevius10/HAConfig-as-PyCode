...
def scrape_housing_factory(provider):
  ...
  def scrape_housing(provider=provider):
    try:
      ...
      store(entity=get_entity(provider), value=[apartment.__string__() for apartment in apartments][:254])
...
def filtering(apartment: Apartment) -> Optional[Apartment]:
  ...
  if apartment.rent is not None and re.findall(r'\d', apartment.rent) and not (400 < int(''.join(re.findall(r'\d', apartment.rent)[:3])) < SET_SCRAPE_HOUSING_FILTER_RENT):
    return resulted(RESULT_STATUS.DISCARDED, message=MAP_RESULT_REASON.FILTERED, details=str(apartment))
  if apartment.text is not None:
...
def scrape(content, item, address_selector, rent_selector, size_selector=None, rooms_selector=None, details_selector=None) -> List[Apartment]:
  ...
  for element in elements:
    ...
    apartment = Apartment(address, rent, size, rooms, details, element_text)
