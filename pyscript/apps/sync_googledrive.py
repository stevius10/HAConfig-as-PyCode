...
class Drive():
  ...
  def list_files(self, folder_id):
    try:
      ...
      logfile.log(f"Error listing files: {e}")
      return {"all": [], "names": []}
  ...
  def download_file(self, filename, local_path, file_id, update=False):
    ...
    try:
      ...
      logfile.log(f"Error downloading file: {e}")
    ...
    try:
      ...
      if existing_file:
        ...
        if modified_timestamp > remote_modified_time:
          ...
          logfile.log("[updated] {filename} from {Utils.get_local_file_timestamp(local_path)} ({local_path})")
        else:
          logfile.log("[skipped] {e}", filename)
      else:
      ...
      logfile.log(f"Error uploading file: {filename}. Error: {e}")
      return False
  ...
  def find_file_by_name(self, filename, folder_id):
    try:
      ...
      logfile.log(f"Error finding file by name: {e}")
      return None
  ...
  def upload_folder(self, foldername, folder_id):
    ...
    try:
      ...
      logfile.log(f"Error creating folder: '{folder_metadata["name"]}'. Error: {e}")
      return False
  ...
  def get_folder_size(self, folder_id):
    ...
    try:
      ...
      logfile.log(f"Error getting folder size: {e}")
      return 0
  
  def synchronize(self, local_path, folder_id, recursive=False):
    logfile.log("{}".format('"[sync] recursively" if recursive else "[sync]"'))
    ...
    try:
      ...
      while queue:
        ...
        if len(same_files) == 0 and not recursive:
          logfile.log("[skipped] {e}", current_local_path)
      ...
      logfile.log(f"Error synchronizing folder: {e}")
  def upload_file(self, filename, local_path, folder_id, update=False):
    try:
      local_path = f"{local_path}/{filename}"
      modified_timestamp = Utils.get_local_file_timestamp(local_path)
      ...
      if existing_file:
        ...
        logfile.log("[uploaded] {e} ({e})", filename, local_path)
  ...
  def find_file_by_name(self, filename, folder_id):
    try:
      ...
      logfile.log(f"Error finding file by name: {filename}. Error: {e}")
      return None
  ...
  def compare_files(self, local_file_data, remote_file_data):
    try:
      ...
      logfile.log(f"Error comparing files: {e}")
      return None
  ...
  def upload_folder(self, foldername, folder_id):
    try:
      ...
      uploaded_folder = self.__service.files().create(body=folder_metadata).execute()
      logfile.log("[created] folder '{e}'", uploaded_folder['name'])
      return uploaded_folder['id']
    except Exception as e:
      logfile.log(f"Error creating folder: '{folder_metadata["name"]}'. Error: {e}")
      return False
  def get_folder_size(self, folder_id):
    try:
      ...
      logfile.log(f"Error getting folder size: {folder_id}. Error: {e}")
      return 0

  async def synchronize(self, local_path, folder_id, recursive=False):
    try:
      logfile.log("{}".format("[sync] recursively" if recursive else "[sync]", local_path))
  ...
  def upload_folder(self, foldername, folder_id):
    try:
      if foldername in SERVICE_GOOGLE_drive_ignore_folders:
        logfile.log("[skipped] Ignored folder: {e}", foldername)
        return False
...
class Utils():
  ...
  def list_local_files(cls, local_path):
    try:
      ...
      logfile.log(f"Error listing local files: {e}")
      return []
  ...
  def get_local_file_timestamp(cls, path):
    try:
      ...
      logfile.log(f"Error getting local file timestamp: {e}")
      return 0
  ...
  def convert_datetime_timestamp(cls, date):
    try:
      ...
      logfile.log(f"Error converting datetime to timestamp: {e}")
      return 0
  ...
  def convert_timestamp_datetime(cls, timestamp):
    try:
      ...
      logfile.log(f"Error converting timestamp to datetime: {e}")
      return ""
...
async def service_googledrive_sync():
  try:
    ...
    pyscript.log(msg=f"Error synchronizing Google Drive: {e}", level=f"Error")
  finally:
    logfile.finished()