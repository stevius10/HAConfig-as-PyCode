...
def scrape_gewobag():
  ...
  if content:
    ...
    for item in items_website:
      ...
      area = item.find(class_='angebot-area').find('td').get_text() if item.find(class_='angebot-area') else None
      rent = item.find(class_='angebot-kosten').find('td').get_text() if item.find(class_='angebot-kosten') else None
      if address:
        items.append(f"{address} ({area}, {rent})")
  return items

if __name__ == "__main__":
  print(', '.join(scrape_gewobag())[:254])